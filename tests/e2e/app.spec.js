...
test('End-to-end user workflow', async () => {
    ...
    let inputField = window.locator('#todo-input');
    await inputField.type(taskText);

    let addButton = window.locator('#add-todo-btn');
    ...
    let newTodo = window.locator('.todo-item');
    let textContent = await newTodo.textContent();
    expect(textContent).toContain(taskText);
    ...
    let checkbox = newTodo.locator('input[type="checkbox"]');
    await checkbox.click();

    let className = await newTodo.getAttribute('class');
    ...
    let deleteBtn = newTodo.locator('.delete-btn');
